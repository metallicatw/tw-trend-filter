...
def test_預設的說明文字和改成可調之前逐字相同():
    ...
    ⚠️ **只有一個字是刻意和舊版不同的：「張」改成「股」。**
    舊版寫死的那一行印的是「> 1,000 張」，而 `min_vol20` 是直接拿去和
    yfinance 的 Volume 比的，那個欄位的單位是**股**（實測 2330 的二十日均量
    18,089,518 股 ＝ 約 18,000 張）。也就是說，那一行從第一天起就在說謊：
    印著一千張，跑的是一張。門檻的數字沒有動（真正在擋流動性的是日均成交
    金額 5,000 萬），動的是那句話——見 tests/test_session.py。
    """
    assert DEFAULT_RULES.describe() == (
        ('① 基礎流動性防禦',
         '股價 > 10 元 ｜ 20日均量 > 1,000 股 ｜ 日均成交金額 > 5,000 萬元'),
        ('② 趨勢多頭確認',
...
def test_量比門檻拉高就擋下來(tmp_path):
    """合成的量比是 900,000 / 615,000 ＝ 1.46。門檻 1.6 就該擋住、1.4 放行。

    分母是**含當天**的 20 日均量（19 × 600,000 ＋ 900,000）÷ 20 ＝ 615,000——
    和桌機版 V3.1 一樣（2026-09-23 改回；見 `tests/test_desktop_parity.py`）。
    """
