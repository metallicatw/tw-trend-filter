...
**第 3 關．突破確認**　收盤價突破布林上軌，或突破前 20 日最高收盤價（Donchian，以收盤計）。
第 2 關說「要有事發生」，這一關要求那件事**已經發生**。

**第 4 關．量能背書**　當日成交量 ÷ 20 日均量（含當日）≥ 1.2。
沒有量的突破是假突破，這一關把它們刷掉。

> 這四關的每一個算式都和桌機版 `TW_Stock_Trend_Following_Trading_Filter_PC_V3.1.py`
> **逐字相同**，由 `tests/test_desktop_parity.py` 逐檔比對守著。同一天、同一份 Yahoo 資料，
> 兩邊篩出來的清單必須一模一樣；要改定義，兩邊一起改。
...
* ⚠️ **價量是「還原」後的序列**（`auto_adjust=True`）：除權息會把**除息之前**
  的價格整批往下調，最後一根的因子是 1.0。用還原價算趨勢指標是合理的做法
  （不然每一次除息都會在均線上留一個假的跳空），但有兩個後果要知道：

  1. 報告上的收盤、均線、布林、停損，和券商軟體的原始價對不起來。
     2330 實測差 0.27%（MA60 2393.52 vs 2400.08）；殖利率 5~6% 的金融股會差
     幾個百分點，而台股的除息集中在 7~9 月。
  2. 偏差的方向是固定的：均線與前高比真實價位低，而收盤是真實價位——
     於是「站上季線」「突破前高」在除息後的一段時間裡**比較容易成立**。
