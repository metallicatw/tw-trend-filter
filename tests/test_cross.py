...
def test_那個入口的兩個門檻預設都是零():
    """0 ＝ 這一關不啟用。

    預設看到的是**技術面四關的完整名單**，六大評分與報酬風險比只在卡片上顯示、
    不拿來篩。

    原本是 3 和 2。那一組很嚴：實測 2026/09/18，25 檔趨勢入選裡只有 1 檔同時過
    另外兩關。而四關本身在弱勢的日子就可能是零檔——兩個嚴格條件疊起來，打開這
    一頁最常看到的是一張空名單，而空名單沒辦法讓人判斷「今天是沒有標的，還是
    門檻太緊」。

    想要那一組的人把數字打上去就有了，畫面會當場重篩；反過來比較難——你得先
    知道有那兩個框，而它們預設就擋掉了大部分的東西。
    """
    assert CROSS_DEFAULTS == {'min_six': 0.0, 'min_rr': 0.0}
    # 鍵名要和輸入框對得上，不然那兩個值填不進去、也不會有人發現。
    keys = {k for k, *_ in LIVE_FIELDS}
    assert set(CROSS_DEFAULTS) <= keys, set(CROSS_DEFAULTS) - keys
